...
const getRestaurantById = async (id) => {
    const restaurant = await restaurants.filter((e) => e.id == id)
    return restaurant
}

const createRestaurant = async (rest) => {
    rest.id = restaurants[restaurants.length-1].id + 1
    restaurants.push(rest)
    return await rest
}

const updateRestaurant = async (id, rest) => {
    const index = restaurants.findIndex((e) => e.id == id)
    if(index == -1){
        return "Error. No se encontró el id."
    } else {
        const newRestaurant = {...restaurants[index], ...rest}
        restaurants.splice(index, 1, newRestaurant)
